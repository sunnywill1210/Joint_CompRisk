...
# Define eoi and cr variable to fit CSH model and OCH model
hd_all$type=hd_all$cens
hd_all$eoi=(hd_all$cens==1)
hd_all$cr=(hd_all$cens==2)
...
sdfcox1 <- coxph(Surv(hd_all$alltime,hd_all$eoi) ~ hd_all$x) # CSH model
...
sdfcox2 <- coxph(Surv(hd_all$alltime,hd_all$cr==1) ~ hd_all$x) # ACH model
teststat[2]=(sdfcox2$coef/sqrt(sdfcox2$var))^2  
...
### maximum test ###
teststat[5]=max(abs(W_1), abs(W_2))
nbootstrap=1000000
...
colnames(pvalue)=c("CSH","OCH","Bonf","Chi","Max")
pvalue
...
text(exp(mean1), exp(mean2)+0.05,"(1.55, 1.70)")

axis(side=1)
axis(side=2)
#title(main="(c)")
legend(list(x=2,y=1.5),c("Chisq", "Bonf","Max"), cex=0.8, lty = c(4,2,1))
